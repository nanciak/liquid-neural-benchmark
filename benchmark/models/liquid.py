...
The ncps recurrent classes are subclassed only to preserve the final
singleton dimension of batched timespans. All cell computations remain
those of the official ncps CfC and LTC implementations.
"""
...
def _prepare_timespans(
    ...
    """Validate elapsed times and align them with the input tensor."""
...
def _validated_lengths(
    ...
    """Return valid sequence lengths on the values device."""
...
def _last_valid_output(
    ...
    """Select the final valid output for each sequence."""
...
def _broadcast_safe_forward(
    ...
    """Run the official ncps recurrence with safe timespan broadcasting.

    This mirrors the official ncps PyTorch forward loop. The only
    behavioral change is replacing `.squeeze()` with `.reshape(-1, 1)`
    for batched elapsed times.
    """

    device = input_tensor.device
    is_batched = input_tensor.dim() == 3
    batch_dim = 0 if layer.batch_first else 1
    sequence_dim = 1 if layer.batch_first else 0
    ...
    if hx is None:
        ...
        if (
            ...
            raise RuntimeError(
                "Running with mixed_memory=True "
                "requires a tuple (h0, c0)."
            )
...
class BroadcastSafeCfC(OfficialCfC):
    """Official ncps CfC with safe batched timespan broadcasting."""
...
class BroadcastSafeLTC(OfficialLTC):
    """Official ncps LTC with safe batched timespan broadcasting."""
